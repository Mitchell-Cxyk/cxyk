...
public class TwoStage {
    ...
    @Contract(pure = true)
    public TwoStage(Matrix a, Fraction[] b, Fraction[] c) {
        A = a;
        this.b = b;
        this.c = c;
    }
    ...
    @Contract(pure = true)
    public TwoStage(Fraction[][] A, Fraction[] b, Fraction[] c) {
        this.A=new Matrix(A);
        this.b = b;
        this.c = c;
    }
    ...
    public static void main(String[] args) {
        ...
        //更新I_B
        I_B= new Integer[]{2, 3};
        I_N=new Integer[]{1,4,5};
        System.out.print("LENGTH:"+I_B.length+" "+I_N.length);
        TwoStage secendStage=new TwoStage(firstStage.A,MyUtil.subArray(result1,I_B),cfraction);
        B=Matrix.makeSubMatrix(secendStage.A,I_row,I_B);
        B.printMatrix();
        Matrix mrivB=Matrix.mriv(B);
        mrivB.printMatrix();
        N=Matrix.makeSubMatrix(secendStage.A,I_row,I_N);
        delta=SimplexStaticMethod.deltaCompute(cfraction,I_B,I_N,B,N);
